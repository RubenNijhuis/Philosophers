...
void	assign_env(t_program_data *pd, char **argv)
{
...
bool	create_locks(t_program_data *pd)
{
...
bool	initiate_table(t_program_data *pd, t_philosopher *philos)
{
	...
	while (i < pd->amount_philos)
	{
		if (pthread_mutex_init(&pd->forks[i], NULL) != 0)
			return (false);
		if (make_philo_thread(philos, pd, (uint32_t)i) == false)
			return (false);
		i++;
	}
	if (create_sim_death_checker(philos) == false)
		return (false);
	return (true);
}
